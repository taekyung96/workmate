...
- 모노레포: `workmate-was`(:8081, REST+JPA+MyBatis+Spring AI, **내부망·v2에서 복사(초기), 이후 필요 시 수정 가능**) / `workmate-web`(:8080, **얇은 BFF** — SPA 서빙+세션+프록시+SSE 중계) / `workmate-vue`(Vue3 SPA)
...
- 이유: 저장소의 커밋 이력을 명확하고 온전하게 관리하기 위해.
