...
-- ── 사용자 계정 (테이블명은 admin_user 지만 관리자 전용이 아니라 전체 사용자다. 구분은 role) ──
COMMENT ON TABLE  admin_user                  IS '사용자 계정 — 일반/관리자를 role 로 구분한다. 이메일·전화번호는 AES-256 암호화 저장';
COMMENT ON COLUMN admin_user.user_seq         IS '사용자 식별자 (PK)';
COMMENT ON COLUMN admin_user.email            IS '로그인 이메일 — AES-256 결정적 암호화(고정 IV). 같은 평문이 같은 암호문이라 UK·검색이 가능해 512자';
COMMENT ON COLUMN admin_user.password         IS '비밀번호 — BCrypt 단방향 해시(60자). 소셜 전용 계정은 임의값';
COMMENT ON COLUMN admin_user.user_name        IS '표시 이름';
COMMENT ON COLUMN admin_user.phone            IS '전화번호 — AES-256 결정적 암호화(고정 IV)';
COMMENT ON COLUMN admin_user.role             IS '권한 — ROLE_USER | ROLE_ADMIN';
COMMENT ON COLUMN admin_user.login_fail_count IS '연속 로그인 실패 횟수 (F1-06) — 임계치 도달 시 locked_at 설정';
COMMENT ON COLUMN admin_user.locked_at        IS '계정 잠금 시각 (F1-06) — 설정 후 lock-minutes 동안 로그인 차단. NULL 이면 잠금 아님';
COMMENT ON COLUMN admin_user.use_yn           IS '사용 여부 — false 는 논리 삭제(행을 지우지 않는다)';
COMMENT ON COLUMN admin_user.created_at       IS '가입 일시';
...
COMMENT ON COLUMN user_social_account.user_seq         IS '연결된 사용자 식별자 (admin_user.user_seq)';
...
COMMENT ON COLUMN chat_room.user_seq   IS '방 소유자 식별자 (admin_user.user_seq)';
...
COMMENT ON COLUMN receipt.user_seq     IS '소유자 식별자 (admin_user.user_seq)';
...
COMMENT ON COLUMN guide.user_seq   IS '작성자 식별자 (admin_user.user_seq)';
...
COMMENT ON COLUMN voice_record.user_seq         IS '소유자 식별자 (admin_user.user_seq)';
