...
<template>
    <div class="mx-auto h-full max-w-5xl overflow-y-auto px-6 py-8">
        ...
        <p v-else-if="users.length === 0" class="py-16 text-center text-sm text-muted-foreground">
            조건에 맞는 사용자가 없습니다.
        </p>
        ...
        <AlertDialog v-model:open="resetOpen">
            <AlertDialogContent>
                <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                        {{ resetTarget?.userName }}님의 비밀번호를 임시 비밀번호로 변경합니다. 임시
                        비밀번호는 이 창을 닫으면 다시 볼 수 없습니다.
                    </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
                    <AlertDialogCancel>취소</AlertDialogCancel>
        ...
        <Dialog :open="tempPassword !== null" @update:open="(v) => !v && (tempPassword = null)">
            <DialogContent>
                <DialogHeader>
                    ...
                    <DialogDescription>
                        아래 임시 비밀번호를 사용자에게 전달하세요. 이 창을 닫으면 다시 확인할 수
                        없습니다.
                    </DialogDescription>
                </DialogHeader>
