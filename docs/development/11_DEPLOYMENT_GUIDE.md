...
Flyway 시드(`V2__seed_reference_data.sql`)는 데모 계정을 **넣지 않는다** — 이메일이
개발용 AES 키로 암호화된 값이라 배포 키로는 복호화가 안 되기 때문이다(아래 참고). 계정 자체
(`user_name`·BCrypt 비밀번호·`ROLE_USER`)와 캡처용 채팅·영수증·회의록 콘텐츠는
`db/init/legacy/13-seed-demo-data.sql` 이 만드는데, 이 파일은 이제 어떤 경로로도 자동 실행되지 않는다.
`scripts/bootstrap-demo-login.sh` 는 **이미 있는 계정의 email·phone 을 재암호화만** 한다 — 계정을
새로 만들지는 않는다.

> ⚠️ **알려진 공백** — Flyway 로 새로 뜬 빈 DB 에는 데모 계정이 아예 없다. `bootstrap-demo-login.sh` 를
> 그대로 돌리면 `WHERE user_name = ...` 이 0건에 매치돼 조용히 아무 일도 하지 않는다(에러도 안 남).
> 지금은 `docker exec -i workmate-db psql -U <user> -d <db> < db/init/legacy/13-seed-demo-data.sql` 로
> 계정·콘텐츠를 먼저 넣은 뒤 `bootstrap-demo-login.sh` 를 돌리는 수동 절차가 필요하다.
> 이 공백을 어떻게 메울지(스크립트에 INSERT 로직 추가 등)는 팀 리드 판단이 필요해 별도로 보고했다.

비밀번호(`Workmate!2026`)가 공개 README 에 적혀 있어, 계정은 기본적으로 `ROLE_USER` 로 둔다 —
관리자 권한이면 누구나 사용자 관리·감사로그에 들어올 수 있기 때문이다.

관리자 승격은 `scripts/bootstrap-demo-login.sh --grant-admin` 을 **사람이 직접 실행할 때만** 된다.
자동으로 켜지는 경로는 없다 — 개발·배포 어느 쪽 compose 도 승격을 자동으로 하지 않는다.
공개 인스턴스에서는 이 플래그를 쓰지 않으므로, 방문자는 채팅·영수증·가이드·회의록까지
체험하고 관리자 화면에는 들어오지 못한다.
