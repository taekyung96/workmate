...
> - **스코프(1단계)**: **오디오 파일 업로드 → 전사 + 3단 요약 → 결과 2분할 표시 + TXT 다운로드**. (2단계: 실시간 녹음·웨이브폼 / 3단계: 이력·가이드/채팅 공유 → 미구현)
> - **프라이버시**: 회의 음성은 민감정보라 **오디오 원본은 저장하지 않고** 전사문·요약만 DB에 남긴다. (원안의 `audio_path`·`duration_seconds` 미사용)
...
| **F8-1-6** | **사내 지식/채팅 공유**  | 요약된 회의록을 즉시 사내 가이드 문서(`guide`)로 등록하거나 AI 채팅(`chat`)에 전송.                                                                                |
