...
4. **데모 계정 부트스트랩이 새 DB 에서 깨진다** — Flyway V2 는 의도적으로 데모 계정을 넣지 않는데,
   `scripts/bootstrap-demo-login.sh` 는 계정을 새로 만들지 않고 기존 계정만 재암호화한다.
   지금은 `db/init/legacy/13-seed-demo-data.sql` 을 수동으로 먼저 적용해야 한다(아래 한계 참고)
...
- **데모 계정이 AES 키에 묶여 있고, Flyway 로 새로 만든 DB 에는 아예 없다.** 계정 자체는
  `db/init/legacy/13-seed-demo-data.sql` 이 만드는데 이 파일은 더 이상 자동 실행되지 않는다
  (Flyway V2 는 의도적으로 데모 데이터를 뺀다 — 개발 키로 암호화된 값이라 다른 배포에서 못 씀).
  `scripts/bootstrap-demo-login.sh` 는 **이미 있는 계정의 email·phone 을 재암호화만** 해서,
  지금은 legacy SQL 을 수동 적용 → 부트스트랩 스크립트 실행 순서를 지켜야 한다. 스크립트가
  계정까지 만들도록 확장할지는 아직 미결정(→ 위 "다음에 할 일" 4번)
