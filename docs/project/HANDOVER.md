...
| 테스트   | **3층** — WAS 156 · WEB 10 / Vue 유닛 73(15개 화면 전부) / E2E 37(실제 브라우저). ↓ "테스트 구성" 참고               |
...
| E2E (실제 브라우저) | 37 | `npm run test:e2e` | **레이아웃·실제 라우팅**. *도우미 패널이 본문을 덮던 문제, 로그인 직후 404 로 떨어지던 문제, **도우미 버튼이 전송 버튼을 덮어 전송이 안 되던 문제***(2026-09-04) |
...
- **프론트 store 직접 테스트** — 유닛 73건·E2E 37건으로 늘었고 store 는 페이지 테스트를 통해
...
**세 결함의 뿌리가 같다 — 넓은 화면에서만 확인한 코드다.** 전송 버튼 겹침(1300px 아래),
사이드바 미접힘(768px 아래), 표 찌그러짐(모바일). 셋 다 데스크탑에서는 멀쩡하고 좁아질수록
드러난다. **E2E 의 desktop 프로젝트 뷰포트가 1440×900 하나뿐**이라 어느 것도 걸리지 않았다.
지금은 각 spec 이 `setViewportSize` 로 폭을 직접 지정해 좁은 쪽을 함께 본다 —
**프로젝트에 1280px 뷰포트를 하나 더 두면 새 화면도 자동으로 걸린다**(미적용, 검토 대상).
