...
| 테스트   | **3층** — WAS 156 · WEB 10 / Vue 유닛 73(15개 화면 전부) / E2E 32(실제 브라우저). ↓ "테스트 구성" 참고               |
...
| E2E (실제 브라우저) | 32 | `npm run test:e2e` | **레이아웃·실제 라우팅**. *도우미 패널이 본문을 덮던 문제, 로그인 직후 404 로 떨어지던 문제, **도우미 버튼이 전송 버튼을 덮어 전송이 안 되던 문제***(2026-09-04) |
...
- **프론트 store 직접 테스트** — 유닛 73건·E2E 32건으로 늘었고 store 는 페이지 테스트를 통해
