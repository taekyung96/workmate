...
| 테스트   | **3층** — WAS 156 · WEB 10 / Vue 유닛 73(15개 화면 전부) / E2E 25(실제 브라우저). ↓ "테스트 구성" 참고               |
...
| 모델     | **멀티 프로바이더 라우팅** — 재시작 없이 Gemini·Groq 전환. 제공자는 `AI_MODEL` 공통코드 `attr1` 이 정한다. **기본은 Groq Qwen3.8 27B** |
| 도우미   | **페이지 인식 챗봇** — 화면 사용법(전 화면) + 사용량 데이터 질문(Tool Calling). 대화는 저장하지 않는다 |
| RAG 품질 | 골든셋 33문항 평가 하네스 + 리포트 2회                                                                                 |

### 테스트 구성 (무엇을 어느 층이 잡나)

세 층이 **서로 다른 종류의 결함**을 잡는다. 층을 나눈 근거는 실제로 겪은 버그들이다 —
아래 표의 "대표 사례" 는 전부 그 층에서만 잡혔거나, 그 층이 없어서 놓쳤던 것들이다.

| 층 | 건수 | 실행 | 잡는 것 · 대표 사례 |
| --- | --- | --- | --- |
| 백엔드 유닛 | WAS 156 · WEB 10 | `./gradlew :workmate-was:test :workmate-web:test` | 라우팅·권한·스트림 처리. *도우미가 제공자를 `null` 로 고정해 Groq 모델을 Gemini 로 보내던 문제* |
| 프론트 유닛·페이지 | 73 | `npm run test:unit` | 렌더 크래시·데이터 배선·화면 규칙. *마스킹된 이메일만 노출, 꺼 둔 코드도 관리 화면엔 보이기, 기본 모델 = 공통코드 첫 번째* |
| E2E (실제 브라우저) | 25 | `npm run test:e2e` | **레이아웃·실제 라우팅**. *도우미 패널이 본문을 덮던 문제, 로그인 직후 404 로 떨어지던 문제* |

**E2E 는 백엔드를 띄우지 않는다.** API 를 브라우저 레벨에서 가로챈다(`workmate-vue/e2e/fixtures.ts`).
실제 WAS 를 띄우면 CI 에 LLM API 키가 필요해지고(키 없이는 기동하지 않는다) 무료 티어 한도를
테스트가 소모한다. SSE 스트리밍도 가로채기로 흉내 낸다 — 실제 모델은 응답이 매번 달라 단언할 수 없다.
서버 계약은 백엔드 층이 지킨다.

**jsdom 이 못 보는 것이 있다.** 폭을 계산하지 않아 "패널이 본문을 가렸는지" 를 알 수 없다.
그래서 페이지 테스트 73건이 전부 통과한 상태로 그 결함이 배포됐다. E2E 는 본문 폭을 실제로 재고,
예전 구현으로 되돌리면 실패하는 것까지 확인했다.

### 다음에 할 일

**끝난 것**: Flyway([ADR-0005](adr/0005-flyway-migration.md)) · 사용량 대시보드(관리자·본인) ·
Redis 분산 상태([ADR-0006](adr/0006-redis-distributed-state.md)) · 페이지 인식 도우미 · 채팅 제공자 전환.

남은 과제:

1. **임베딩 로컬화 검토** — RAG 를 켠 요청은 Gemini 임베딩 한도에 걸린다. 실사용에서 실제로
   문제가 되면 ONNX 내장(`spring-ai-starter-model-transformers`) 또는 Ollama 로 내린다.
   **평가 하네스로 Hit@K·MRR 전후 비교가 선행돼야 한다**(현재 100% · 0.970)

그 밖에 계속 남아 있는 것:

- **도메인 연결** — 확보하면 [12. 운영 가이드 §8](../development/12_OPERATIONS.md) 대로 고정 터널로 전환.
  소셜 로그인 콜백도 이때 등록할 수 있다
- **프론트 store 직접 테스트** — 유닛 73건·E2E 25건으로 늘었고 store 는 페이지 테스트를 통해
  간접적으로 돈다(채팅·영수증·회의록). 다만 **store 를 직접 겨냥한 테스트는 아직 없다** —
  ADR-0002 의 "로직은 composable/store" 중 store 쪽은 여전히 간접 증명이다
- **부하 테스트 재측정** — 1차 측정은 끝났다([리포트](../features/loadtest/REPORT-2026-09-04.md)).
  다만 **생성기와 대상이 같은 머신**이라 처리량 비교가 성립하지 않았고, 테스트가 **세션 하나를 공유**해
  Redis 키 경합이 그 자체로 병목이 됐다. 제대로 재려면 생성기를 분리하고 세션을 여러 개로 흩어야 한다
...
- **비용은 추정치다.** `app.usage.pricing` 의 단가로 계산하며 실제 청구액이 아니다.
  `model_name` 에는 별칭(`gemini-flash-latest`)이 아니라 **제공자가 해석한 실제 이름**(`gemini-3.8-flash`)이
  저장되므로, 별칭이 새 버전을 가리키면 단가를 추가해야 한다 — 그전까지는 "단가 미등록 N건" 으로 드러난다.
  **실제로 겪었다**: 별칭이 3.7 → 3.8 로 옮겨가며 5건이 "단가 미등록" 으로 빠졌고, 그 표시 덕에 발견했다
- **Groq `gpt-oss-120b` 는 도구 호출에서 빈 응답을 준다.** 도구가 필요한 질문("내 사용량 얼마나 썼어?")에
  **예외 없이** 빈 스트림을 돌려주고 `[Tool]` 로그조차 남지 않는다. 같은 질문을 `qwen3.8-27b` 로 보내면
  정상 호출된다(재현 확인). 일반 대화는 멀쩡해 더 드러나지 않는다. 그래서 `AI_MODEL` 공통코드에서
  `use_yn = false` 로 목록에서만 내렸다 — 과거 기록(`llm_usage.model_name`)을 설명해야 해서 코드는 남겼다.
  고쳐지면 관리자 > 공통 코드 화면에서 다시 켜면 된다.
  **빈 응답 자체는 이제 침묵하지 않는다** — 토큰이 하나도 없으면 저장하지 않고 오류로 알린다
- **도우미의 데이터 질문은 사용량 2개 화면만 지원한다.** 나머지 화면은 사용법만 답한다.
  화면별로 `@Tool` 을 더하면 확장되는 구조다
- **무료 티어 한도가 응답 속도를 지배한다.** Gemini 무료 티어는 분당 한도가 빡빡해
  429 가 잦고, 그때마다 재시도 대기가 붙는다. 실측: 채팅 25~70초.
  그래서 **기본 모델을 Groq Qwen3.8 27B 로 두었다** — 실측 0.6초 vs Gemini 11초.
  화면 드롭다운의 기본값은 `AI_MODEL` 공통코드의 `sort_order` 첫 번째이고, 서버 기본값(도우미·
  `modelCode` 없는 호출)은 `LLM_CHAT_MODEL` 이다. **"기본 모델" 이 두 군데라 함께 맞춰야 한다.**
  어느 모델이 어느 제공자인지는 같은 공통코드의 `attr1` 이 정한다.
...
채팅이 느리고 답변이 안 오는 문제를 추적하다 **세 층에 걸친 결함**을 찾아 고쳤고,
그 과정에서 드러난 테스트 공백을 메웠다.

| PR                                                    | 내용                                                                                                        |
| ----------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- |
| [#24](https://github.com/taekyung96/workmate/pull/24) | 기본 모델을 Groq Qwen3.8 27B 로 · 모델과 제공자를 함께 정하는 `ChatModelResolver` · 빈 응답을 오류로 알림 |
| [#25](https://github.com/taekyung96/workmate/pull/25) | 도우미 패널 전환 효과 · **없는 주소로 로그인하면 첫 화면이 404** 였던 가드 문제                             |
| [#26](https://github.com/taekyung96/workmate/pull/26) | 페이지 스모크 테스트 73건(15개 화면) · `chat.store.loadModels` 의 unhandled rejection                        |
| [#27](https://github.com/taekyung96/workmate/pull/27) | E2E 25건 — 실제 브라우저로 레이아웃·라우팅 검증(백엔드 없이)                                                |
| [#28](https://github.com/taekyung96/workmate/pull/28) | 채팅 답변 폭·표 렌더링 · **원본이 사라진 임베딩(고아 청크)** 정리(V8)                                       |

특히 기억할 것 두 가지.

- **`LLM_CHAT_MODEL` 하나가 Gemini 클라이언트의 모델까지 정하고 있었다.** OCR·음성 전사·회의 요약은
  그 클라이언트를 모델 옵션 없이 쓰기 때문에, 채팅 기본 모델을 Groq 으로 바꾸면 세 기능이 함께
  깨질 상태였다. `LLM_MULTIMODAL_MODEL` 로 분리했다
- **E2E 목이 `auto` 가 아니면 테스트가 진짜 서버로 샌다.** 목을 요청하지 않은 테스트의 `/api` 호출이
  `vite preview` 프록시를 타고 나가, 개발 머신에서는 백엔드가 떠 있어 통과하고 CI 에서만 502 로 깨졌다.
  백엔드를 내린 채 25건이 통과하는 것으로 격리를 확인했다
