...
2. **도우미 실제 동작 확인** — 구현·단위테스트는 끝났으나 Gemini 무료 할당량(429) 때문에 라이브 검증이 남았다
...
- **도우미 라이브 검증이 남아 있다.** 단위 테스트는 LLM 을 목킹하므로 Tool Calling 이 실제로
  도는지, 권한 거부가 실제로 되는지는 확인하지 못했다 (Gemini 429 로 막힘)
